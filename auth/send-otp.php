...
/**
 * Get access token from SMS API login
...
function getSMSAccessToken() {
    ...
    if ($curlError || $httpCode !== 200) {
        error_log("SMS API Login Error: HTTP $httpCode - $curlError");
        return false;
    }
...
/**
 * Send OTP via SMS using the SMS API (with dynamic token)
 * @param string $phone - Staff phone number (format: 971XXXXXXXXX)
 * @param string $otp - 6-digit OTP code
 * @param string $staffName - Staff name for personalization
 * @return array - Status of SMS sending
 */
function sendOTPViaSMS($phone, $otp, $staffName) {
    // Get fresh access token
    $accessToken = getSMSAccessToken();
    
    if (!$accessToken) {
        return [
            'sent' => false,
            'message' => 'Failed to authenticate with SMS API'
        ];
    }
    
    // SMS API endpoint
    $smsUrl = 'https://nexus.eandenterprise.com/api/v1/sms/send';
    
    // Prepare message
    $message = "Your Selab Nadiry login OTP is: $otp\n\nValid for 10 minutes. Do not share this code.\n\nSelab Nadiry Travels";
    ...
    if ($httpCode === 200 || $httpCode === 201 || $httpCode === 202) {
        ...
        return [
            'sent' => false,
            'message' => 'SMS API returned error: ' . $httpCode,
            'response' => $result
        ];
    }
}
...
// Get JSON input
    ...
    $query = "SELECT staff_id, staff_name, staff_pic, staff_email, staff_phone FROM staff WHERE LOWER(staff_email) = LOWER(:email) AND status = 1";
    ...
    try {
        ...
        $smsStatus = ['sent' => false, 'message' => 'No phone number'];
        if (!empty($user['staff_phone']) && strlen($user['staff_phone']) >= 10) {
            try {
                $smsStatus = sendOTPViaSMS($user['staff_phone'], $otp, $user['staff_name']);
                @file_put_contents($logFile, date('Y-m-d H:i:s') . " - SMS Status: " . json_encode($smsStatus) . "\n", FILE_APPEND);
            } catch (Exception $smsError) {
                // Log SMS error but don't fail the whole request
                @file_put_contents($logFile, date('Y-m-d H:i:s') . " - SMS Error: " . $smsError->getMessage() . "\n", FILE_APPEND);
                $smsStatus = ['sent' => false, 'message' => 'SMS send failed'];
            }
        }
