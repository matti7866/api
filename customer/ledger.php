...
try {
    ...
    if ($action == 'getLedger') {
        ...
        if (empty($checkCustomerRecord)) {
            ...
            // Build SQL dynamically based on whether it's AED or not
            $sql = "SELECT * FROM (
                ...
                // Only include residence transactions if NOT AED
                (!$isAED ? "
                UNION ALL
                ...
                UNION ALL
                SELECT rcc.id AS refID, CONCAT('Residence Extra: ', rcc.charge_title) AS TRANSACTION_Type,
                residence.passenger_name AS Passenger_Name, rcc.created_at AS datetime,
                DATE(rcc.created_at) AS nonFormatedDate,
                DATE_FORMAT(DATE(rcc.created_at), '%d-%b-%Y') AS date,
                country_names AS Identification, '' AS Orgin, '' AS Destination,
                rcc.sale_price AS Debit, 0 AS Credit
                FROM residence_custom_charges rcc
                INNER JOIN residence ON residence.residenceID = rcc.residence_id
                INNER JOIN country_name ON country_name.country_id = residence.VisaType
                WHERE residence.customer_id = :id AND rcc.currency_id = :CurID" : "") . "
                UNION ALL
                ...
                // For AED, exclude residence payments
                ($isAED ? " AND customer_payments.PaymentFor IS NULL AND customer_payments.residenceFinePayment IS NULL" : "") . "
                UNION ALL
            ...
            $sql = "SELECT * FROM (
                ...
                INNER JOIN country_name ON country_name.country_id = residence.VisaType
                WHERE residence.customer_id = :id AND rcc.currency_id = :CurID
                AND DATE(rcc.created_at) BETWEEN :from_date AND CURDATE()" : "") . "
                UNION ALL
                SELECT servicedetails.serviceDetailsID AS refID, serviceName AS TRANSACTION_Type,
                passenger_name AS Passenger_Name, service_date AS datetime,
                DATE(service_date) AS nonFormatedDate, DATE_FORMAT(DATE(service_date), '%d-%b-%Y') AS date,
