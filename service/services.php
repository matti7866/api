...
try {
    ...
    if ($action == 'updateServiceCharge') {
        ...
        // Build SQL - add service_payment_date if account/credit card is assigned
        $sql = "UPDATE servicedetails SET
        ...
        // If account or credit card is assigned, set payment date with current Dubai timestamp
        if ($account_value !== null) {
            $sql .= ", service_payment_date = CURRENT_TIMESTAMP";
        } else {
            // If only supplier is assigned (no account), clear payment date
            $sql .= ", service_payment_date = NULL";
        }
        
        $sql .= " WHERE serviceDetailsID = :id";
