...
try {
    ...
    // 9b. CUSTOMER WALLET DEPOSITS/REFUNDS (Money coming INTO company account from customer)
    if($typeFilter == '' || $typeFilter == 'credit') {
        try {
            $sql = "SELECT 
                        cwt.transaction_id as id,
                        cwt.datetime as transaction_date,
                        CONCAT('Wallet ', CASE WHEN cwt.transaction_type = 'refund' THEN 'Refund' ELSE 'Deposit' END) as transaction_type,
                        'credit' as type_category,
                        cwt.account_id as accountID,
                        cwt.amount,
                        cwt.currency_id as currencyID,
                        cwt.remarks,
                        cwt.customer_id as reference_id,
                        COALESCE(s.staff_name, 'Unknown Staff') as staff_name,
                        CONCAT('Wallet ', CASE WHEN cwt.transaction_type = 'refund' THEN 'Refund' ELSE 'Deposit' END, ' - ', COALESCE(c.customer_name, 'Unknown Customer'), ' (', IFNULL(c.wallet_account_number, 'N/A'), ')') as description
                    ...
                    WHERE cwt.transaction_type IN ('deposit', 'refund')
                    AND cwt.account_id IS NOT NULL
                    AND DATE(cwt.datetime) BETWEEN :fromDate AND :toDate" . $accountCondition;
            ...
            error_log('Error loading wallet deposits: ' . $e->getMessage());
        }
    }
    ...
    // 11b. CUSTOMER WALLET WITHDRAWALS (Money going OUT from company account to customer)
    if($typeFilter == '' || $typeFilter == 'debit') {
        try {
            $sql = "SELECT 
                        cwt.transaction_id as id,
                        ...
                        cwt.customer_id as reference_id,
                        COALESCE(s.staff_name, 'Unknown Staff') as staff_name,
                        CONCAT('Wallet Withdrawal - ', COALESCE(c.customer_name, 'Unknown Customer'), ' (', IFNULL(c.wallet_account_number, 'N/A'), ')') as description
                    FROM customer_wallet_transactions cwt
                    LEFT JOIN customer c ON cwt.customer_id = c.customer_id
                    LEFT JOIN staff s ON cwt.staff_id = s.staff_id
                    WHERE cwt.transaction_type = 'withdrawal'
                    AND cwt.account_id IS NOT NULL
                    AND DATE(cwt.datetime) BETWEEN :fromDate AND :toDate" . $accountCondition;
            ...
            error_log('Error loading wallet withdrawals: ' . $e->getMessage());
        }
    }
    ...
    // 13. SERVICE PAYMENTS (Credit Card & Account Charges)
    if($typeFilter == '' || $typeFilter == 'debit') {
        $sql = "SELECT 
                    sd.serviceDetailsID as id,
                    COALESCE(sd.service_payment_date, sd.service_date) as transaction_date,
                    'Service Payment' as transaction_type,
                    'debit' as type_category,
                    sd.accoundID as accountID,
                    sd.netPrice as amount,
                    sd.netCurrencyID as currencyID,
                    CONCAT(COALESCE(s.serviceName, 'Service'), ' - ', sd.service_details) as remarks,
                    sd.customer_id as reference_id,
                    st.staff_name,
                    CONCAT('Service: ', COALESCE(s.serviceName, 'Unknown'), ' - ', COALESCE(sd.passenger_name, COALESCE(c.customer_name, 'Unknown Customer')), ' (Customer: ', COALESCE(c.customer_name, 'Unknown'), ')') as description
                ...
                WHERE sd.accoundID IS NOT NULL 
                AND sd.netPrice > 0
                AND DATE(COALESCE(sd.service_payment_date, sd.service_date)) BETWEEN :fromDate AND :toDate" . $excludeAccount25_service . str_replace('accountID', 'accoundID', str_replace(':accountID', ':serviceAccountID', $accountCondition));
