...
try {
    ...
    foreach ($accounts as $account) {
        ...
        // Wallet Deposits/Refunds (money coming in from customer to company account)
        $stmt = $pdo->prepare("SELECT COALESCE(SUM(amount), 0) as total FROM customer_wallet_transactions 
                               WHERE account_id = ? AND account_id IS NOT NULL 
        ...
        // Wallet Withdrawals (money going out from company account to customer)
        $stmt = $pdo->prepare("SELECT COALESCE(SUM(amount), 0) as total FROM customer_wallet_transactions 
                               WHERE account_id = ? AND account_id IS NOT NULL 
