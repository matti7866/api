...
if ($action == 'moveResidenceToStep') {
    ...
    try {
        // Map step names to the completedStep value when you are ON that step (not completed)
        // When moving TO a step, you haven't completed it yet, so completedStep is one less
        $stepToCurrentMapping = [
            '1' => 0,      // On Offer Letter (step 0 completed)
            '1a' => 2,     // On Offer Letter Submitted - special case
            '2' => 2,      // On Insurance (step 1a/2 completed)
            '3' => 3,      // On Labour Card (step 2 completed)
            '4' => 4,      // On E-Visa (step 3 completed)
            '4a' => 5,     // On E-Visa Submitted - special case
            '5' => 5,      // On Change Status (step 4/4a completed)
            '6' => 6,      // On Medical (step 5 completed)
            '7' => 7,      // On Emirates ID (step 6 completed)
            '8' => 8,      // On Visa Stamping (step 7 completed)
            '9' => 9,      // On Contract Submission (step 8 completed)
            '10' => 10     // Completed (step 9 completed)
        ];

        $completedStep = isset($stepToCurrentMapping[$targetStep]) ? $stepToCurrentMapping[$targetStep] : 0;
        ...
        if ($targetStep == '1a') {
            ...
            // Regular step move - set completedStep to current position
