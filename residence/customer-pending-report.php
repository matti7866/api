        ...
        $sql = "SELECT * FROM (
            SELECT 
                ...
                (
                    ...
                    -- Custom charges
                    " . $customChargesQuery . " +
                    -- Family residence charges (only include family residences linked to main residences with matching currency)
                    (SELECT IFNULL(SUM(fr.sale_price), 0) FROM family_residence fr 
                        ...
                        WHERE fr.customer_id = main_customer AND r_main.saleCurID = :currencyID)
                ) - (
        ...
        $sql = "SELECT * FROM (
            SELECT 
                ...
                (
                    ...
                    (SELECT IFNULL(SUM(fr.sale_price), 0) FROM family_residence fr 
                        INNER JOIN residence r_main ON r_main.residenceID = fr.residence_id
                        WHERE fr.customer_id = :customer_id AND r_main.saleCurID = :currencyID)
                ) - (
