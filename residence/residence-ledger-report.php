...
try {
    ...
    $sql = "SELECT 
        ...
        CASE 
            WHEN r.current_status = 'cancelled' OR r.current_status = 'cancelled & replaced' THEN 
                (IFNULL((SELECT SUM(cp.payment_amount) FROM customer_payments cp 
                    WHERE cp.PaymentFor = r.residenceID AND cp.currencyID = :CurID 
                    AND (cp.payment_type = 'residence' OR cp.payment_type IS NULL)), 0) +
                 IFNULL((SELECT SUM(cp.payment_amount) FROM customer_payments cp 
                    WHERE cp.residenceCancelPayment = r.residenceID AND cp.currencyID = :CurID), 0))
            ELSE 
                IFNULL((SELECT SUM(cp.payment_amount) FROM customer_payments cp 
                    WHERE cp.PaymentFor = r.residenceID AND cp.currencyID = :CurID 
                    AND (cp.payment_type = 'residence' OR cp.payment_type IS NULL)), 0)
        ...
        IFNULL((SELECT SUM(cp.payment_amount) FROM customer_payments cp 
            ...
            WHERE rf.residenceID = r.residenceID AND cp.currencyID = :CurID), 0) AS finePayment,
        r.current_status,
        IFNULL((SELECT SUM(rc.cancellation_charges) FROM residence_cancellation rc 
            WHERE rc.residence = r.residenceID), 0) AS cancellation_charges,
        ...
        IFNULL((SELECT SUM(cp.payment_amount) FROM customer_payments cp 
            WHERE cp.PaymentFor = r.residenceID AND cp.currencyID = :CurID AND cp.payment_type = 'tawjeeh'), 0) AS tawjeeh_payments,
        ...
        IFNULL((SELECT SUM(cp.payment_amount) FROM customer_payments cp 
            WHERE cp.PaymentFor = r.residenceID AND cp.currencyID = :CurID 
            AND cp.payment_type IN ('insurance', 'insurance_fine')), 0) AS iloe_payments,
        -- Custom charges
