...
$paymentMethod = isset($input['paymentMethod']) ? $input['paymentMethod'] : 'account';
$remarks = isset($input['remarks']) ? trim($input['remarks']) : '';

// Validation
if (!$familyResidenceID || $paymentAmount <= 0) {
    JWTHelper::sendResponse(400, false, 'Invalid payment data. Family Residence ID and payment amount are required.');
}

// Only require accountID for account payments
...
if ($paymentMethod === 'wallet') {
    // Get or create wallet payments account
    ...
    $accountID = $walletAccount ? $walletAccount['account_ID'] : 38; // Fallback to 38 if not found
}
