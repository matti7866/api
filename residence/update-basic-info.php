...
// Extract optional fields - Financial
...
// Extract optional fields - Passenger Information
$passengerName = isset($data['passenger_name']) ? trim($data['passenger_name']) : null;
$passportNumber = isset($data['passportNumber']) ? trim($data['passportNumber']) : null;
$passportExpiryDate = isset($data['passportExpiryDate']) ? $data['passportExpiryDate'] : null;
$gender = isset($data['gender']) ? trim($data['gender']) : null;
$dob = isset($data['dob']) ? $data['dob'] : null;
$uid = isset($data['uid']) ? trim($data['uid']) : null;

// Extract optional fields - Customer Reassignment
$customerId = isset($data['customer_id']) ? (int)$data['customer_id'] : null;

try {
    // Verify residence exists and get customer_id
    $sql = "SELECT residenceID, customer_id FROM residence WHERE residenceID = :residenceID";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':residenceID', $residenceID);
    $stmt->execute();
    $residenceData = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$residenceData) {
        JWTHelper::sendResponse(404, false, 'Residence not found');
    }
    
    $customerId = $residenceData['customer_id'];
    
    // Begin transaction
    $pdo->beginTransaction();
    
    // Reassign to different customer if customer_id provided
    if ($customerId !== null && $customerId != $residenceData['customer_id']) {
        // Verify new customer exists
        $checkStmt = $pdo->prepare("SELECT customer_id FROM customer WHERE customer_id = :customer_id");
        $checkStmt->bindParam(':customer_id', $customerId);
        $checkStmt->execute();
        
        if ($checkStmt->rowCount() === 0) {
            $pdo->rollBack();
            JWTHelper::sendResponse(400, false, 'Selected customer does not exist');
        }
        
        // Update residence customer_id
        $sql = "UPDATE residence SET customer_id = :customer_id WHERE residenceID = :residenceID";
        $stmt = $pdo->prepare($sql);
    ...
    if ($passengerName !== null) {
        $residenceUpdates[] = "passenger_name = :passengerName";
        $residenceParams[':passengerName'] = $passengerName;
    }
    
    if ($passportNumber !== null) {
        $residenceUpdates[] = "passportNumber = :passportNumber";
        $residenceParams[':passportNumber'] = $passportNumber;
    }
    
    if ($passportExpiryDate !== null) {
        $residenceUpdates[] = "passportExpiryDate = :passportExpiryDate";
        $residenceParams[':passportExpiryDate'] = $passportExpiryDate;
    }
    
    if ($gender !== null) {
        $residenceUpdates[] = "gender = :gender";
        $residenceParams[':gender'] = $gender;
    }
    
    if ($dob !== null) {
        $residenceUpdates[] = "dob = :dob";
        $residenceParams[':dob'] = $dob;
    }
    
    if ($uid !== null) {
        $residenceUpdates[] = "uid = :uid";
        $residenceParams[':uid'] = $uid;
    }
    ...
    JWTHelper::sendResponse(200, true, 'Residence information updated successfully', [
        ...
        'updated' => array_merge(
            ...
            $passengerName !== null ? ['passenger_name' => $passengerName] : [],
            $passportNumber !== null ? ['passportNumber' => $passportNumber] : [],
            $passportExpiryDate !== null ? ['passportExpiryDate' => $passportExpiryDate] : [],
