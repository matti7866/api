...
try {
    ...
    $sql = "SELECT 
                ...
                COALESCE((SELECT SUM(payment_amount) FROM customer_payments WHERE PaymentFor = r.residenceID AND (payment_type = 'residence' OR payment_type IS NULL)), 0) AS total_paid,
                ...
                COALESCE((SELECT SUM(payment_amount) FROM customer_payments WHERE PaymentFor = r.residenceID AND payment_type = 'tawjeeh'), 0) AS tawjeeh_payments,
                COALESCE((SELECT SUM(payment_amount) FROM customer_payments WHERE PaymentFor = r.residenceID AND payment_type IN ('insurance', 'insurance_fine')), 0) AS iloe_payments,
                ...
                CASE 
                    WHEN r.completedStep = 9 THEN 'Completed'
    ...
    if ($search) {
        $sql .= " AND (
            LOWER(r.passenger_name) LIKE LOWER(:search)
            OR LOWER(c.customer_name) LIKE LOWER(:search)
            OR LOWER(r.passportNumber) LIKE LOWER(:search)
            OR LOWER(r.uid) LIKE LOWER(:search)
            OR LOWER(comp.company_name) LIKE LOWER(:search)
            OR LOWER(r.EmiratesIDNumber) LIKE LOWER(:search)
            OR CAST(r.residenceID AS CHAR) LIKE :search
        )";
