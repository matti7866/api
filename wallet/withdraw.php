 ...
 * Endpoint: /api/wallet/withdraw.php
 */
...
try {
    ...
    // Get current wallet balance and wallet account number
    ...
    if ($balanceBefore < $amount) {
        $pdo->rollBack();
        JWTHelper::sendResponse(400, false, 'Insufficient wallet balance. Available: ' . $balanceBefore);
    }
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES 
        (:customerID, 'withdrawal', :amount, :currencyID, :balanceBefore, :balanceAfter, :staffID, :accountID, :remarks)
    ");
    ...
    JWTHelper::sendResponse(200, true, 'Funds withdrawn successfully', $response);
    
} catch (Exception $e) {
    if ($pdo->inTransaction()) {
        $pdo->rollBack();
    }
    error_log('Withdraw Funds Error: ' . $e->getMessage());
    JWTHelper::sendResponse(500, false, 'Error withdrawing funds: ' . $e->getMessage());
}
