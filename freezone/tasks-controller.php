...
if (!in_array($action, ['seteVisa', 'seteVisaAccept', 'rejectEVisa', 'setChangeStatus', 'setMedical', 'setEmiratesID', 'setVisaStamping', 'addFreezoneResidence', 'moveFreezoneToStep'])) {
...
try {
    ...
    if ($action == 'moveFreezoneToStep') {
        ...
        // Freezone step mapping:
        // '1' = E-Visa (completedStep = 0)
        // '1a' = E-Visa Submitted (completedStep = 1, eVisaStatus = 'submitted')
        // '2' = Change Status (completedStep = 2)
        // '3' = Medical (completedStep = 3)
        // '4' = Emirates ID (completedStep = 4)
        // '5' = Visa Stamping (completedStep = 5)
        // '6' = Completed (completedStep = 6)
        
        $stepToCompletedMapping = [
            '1' => 0,      // On E-Visa
            '1a' => 1,     // On E-Visa Submitted
            '2' => 2,      // On Change Status
            '3' => 3,      // On Medical
            '4' => 4,      // On Emirates ID
            '5' => 5,      // On Visa Stamping
            '6' => 6       // Completed
        ];

        $completedStep = isset($stepToCompletedMapping[$targetStep]) ? $stepToCompletedMapping[$targetStep] : 0;
        
        // Handle special case for step 1a (E-Visa Submitted)
        if ($targetStep == '1a') {
            $stmt = $pdo->prepare("UPDATE residence SET completedStep = 1, eVisaStatus = 'submitted' WHERE residenceID = :id AND res_type = 'Freezone'");
            $stmt->execute(['id' => $id]);
        } else {
            // Regular step move
            $stmt = $pdo->prepare("UPDATE residence SET completedStep = :completedStep WHERE residenceID = :id AND res_type = 'Freezone'");
            $stmt->execute(['completedStep' => $completedStep, 'id' => $id]);
        }

        JWTHelper::sendResponse(200, true, 'Freezone residence moved to step ' . $targetStep . ' successfully');
    }
