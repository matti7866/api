...
try {
    ...
    // Build query - Using LEFT JOIN to include tickets even if related records are missing
    $sql = "SELECT 
                t.ticket,
                t.ticketNumber,
                t.Pnr,
                ...
                c.customer_phone,
                COALESCE(f.airport_code, 'N/A') as from_code,
                COALESCE(to_airport.airport_code, 'N/A') as to_code,
                COALESCE(curr.currencyName, 'AED') as currency_name,
                COALESCE(net_curr.currencyName, 'AED') as net_currency_name,
                COALESCE(s.supp_name, 'No Supplier') as supplier_name,
                st.staff_name
            FROM ticket t
            LEFT JOIN customer c ON c.customer_id = t.customer_id
            LEFT JOIN airports f ON f.airport_id = t.from_id
            LEFT JOIN airports to_airport ON to_airport.airport_id = t.to_id
            LEFT JOIN currency curr ON curr.currencyID = t.currencyID
            LEFT JOIN currency net_curr ON net_curr.currencyID = t.net_CurrencyID
            LEFT JOIN supplier s ON s.supp_id = t.supp_id
            LEFT JOIN staff st ON st.staff_id = t.staff_id
            WHERE 1=1";
    
    $params = [];
    
    // Apply filters - Search by booking date (datetime) instead of travel date
    if ($startDate) {
        $sql .= " AND DATE(t.datetime) >= :startDate";
        $params[':startDate'] = $startDate;
    }
    
    if ($endDate) {
        $sql .= " AND DATE(t.datetime) <= :endDate";
